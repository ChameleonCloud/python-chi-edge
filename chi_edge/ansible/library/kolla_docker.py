...
class DockerWorker(object):
    def __init__(self, module):
        ...
        self._cgroupns_mode_supported = StrictVersion(
            self.dc._version
        ) >= StrictVersion("1.41")
    ...
    def compare_container(self):
        container = self.check_container()
        if not container or self.check_container_differs() or self.compare_config():
            self.changed = True
        return self.changed
    ...
    def compare_cap_add(self, container_info):
        ...
        current_cap_add = container_info["HostConfig"].get("CapAdd", list())
    ...
    def compare_security_opt(self, container_info):
        ...
        current_sec_opt = container_info["HostConfig"].get("SecurityOpt", list())
    ...
    def compare_cgroupns_mode(self, container_info):
        ...
        if new_cgroupns_mode is None:
            ...
            return False
        current_cgroupns_mode = container_info["HostConfig"].get("CgroupnsMode")
...
def main():
    ...
    try:
        ...
        module.fail_json(
            changed=True, msg=repr(traceback.format_exc()), **getattr(dw, "result", {})
        )
